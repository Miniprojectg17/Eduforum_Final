...
export default function FacultyProfile() {
  ...
  const { data: stats } = useSWR<{
    stats: { studentsManaged: number; resourcesUploaded: number; announcementsMade: number; postsVerified: number }
  ...
  async function handleSave() {
    ...
    if (!res.ok) {
      console.log("[v0] Save faculty error", json)
      return
    }
    mutate()
  }
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-balance">Faculty Profile</h1>
